...
void Shutdown()
{
    ...
    RenameThread("ronpaulcoin-shutoff");
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateRonPaulCoins(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ronpaulcoin.conf)") + "\n";
    strUsage += "  -datadir=<dir>         " + _("Specify data directory") + "\n";
    strUsage += "  -testnet               " + _("Use the test network") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ronpaulcoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 9027 or testnet: 19027)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9026 or testnet: 19443)") + "\n";
...
{
    RenameThread("ronpaulcoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single RonPaulCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. RonPaulCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("RonPaulCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
        ...
        do {
            try {
                ...
                if (!mapBlockIndex.empty() && chainActive.Genesis() == NULL) {
                    LogPrintf("Aborted database. Exiting.\n\n [%s]", GetDataDir());
    ...
    if (pwalletMain)
        GenerateRonPaulCoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif

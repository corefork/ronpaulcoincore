...
{
   int64_t nSubsidy = 1.0 * COIN;
   
    // Subsidy is cut in half every 1051200 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / Params().SubsidyHalvingInterval()); // RonPaulCoin: 1051k blocks in ~4 years
 ...
 int64_t nTargetTimespan = 1.0 * 24 * 60 * 60; // RonPaulCoin: 1.0 day
static const int64_t nTargetSpacing = 2.0 * 60; // RonPaulCoin: 2.0 minutes
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if((nHeight > nKGWDiffFork) || (TestNet() && (nHeight > 250))) // set KGW+TW on testnet to start @ block 250
    {
         static const int64_t  BlocksTargetSpacing  = nTargetSpacing;
         unsigned int  TimeDaySeconds  = 60 * 60 * 24;
         int64_t  PastSecondsMin  = TimeDaySeconds * 0.2;
         int64_t  PastSecondsMax  = TimeDaySeconds * 6;
    ...
    int nDiffFork = 13650;
    if((nHeight >= nDiffFork) || (TestNet() && (nHeight >= 2016)))
...
void ThreadScriptCheck() {
    RenameThread("ronpaulcoin-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.GetBlockTime() < 1376568000)  // stop enforcing 15 August 2013 00:00:00
    {

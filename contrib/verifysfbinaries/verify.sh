...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ronpaulcoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[ronpaulcoin-]<version>-[rc[0-9]] (example: ronpaulcoin-0.7.1-rc1)"
   echo "wget output:"
